    ...
    for row_idx in range(2, ws.max_row + 1):
        for col_idx in match_cols:
            ...
            if ws.cell(row=row_idx, column=col_idx).value == '✗':
                ws.cell(row=row_idx, column=col_idx-2).fill = red_fill  # Pre Value
                ws.cell(row=row_idx, column=col_idx-1).fill = red_fill  # Post Value
                ws.cell(row=row_idx, column=col_idx).fill = red_fill    # Status Cell
